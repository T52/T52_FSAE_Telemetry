...
//===========================General Manual Serial Flush function===========================
void Flush_buffer(){
  ...
  while(serialPort.available() > 0){
     print(".");
     error_buf = serialPort.readBytes();
     serialPort.readBytes(error_buf);
  }
...
//  public:
    ...
    void Readcommand(){
      ...
      while(serialPort.available() == 3){
        serialPort.readBytesUntil('\r', tStor);
      }
...
void xbeesetupfunct() {
  ...
  serialPort.clear(); //Clean yourself up...
  Flush_buffer();
  ...
  println("Outside loop, Configuration complete");
  //exit();
